...
function authorizedFromGrant() {
  return authorizeOpusV2EvalV2PaidAuthorization(
    cloneOpusV2EvalV2PaidAuthorization(),
    OPUS_V2_EVAL_V2_EXPLICIT_GRANT,
  );
}

describe("isolated Opus eval @v2 explicit authorization lifecycle", () => {
  it("keeps the canonical prepared object immutable and derives explicit state", () => {
    ...
    assert.throws(
      () =>
        projectOpusV2EvalV2PaidAuthorization(OPUS_V2_EVAL_V2_PAID_AUTHORIZATION, "authorize", {
          grant: OPUS_V2_EVAL_V2_EXPLICIT_GRANT,
        }),
      /canonical_prepared_authorization_is_immutable/,
    );
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION.status, "explicitly_authorized");
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION.spend_authorized, true);
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION.ceiling_authorized_for_spend, true);
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION.authorized_to_run, true);
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION.bound_workflow_id, null);
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION.hard_cost_ceiling_usd, 5);
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION_STORAGE.kind, "code_bound_explicit_authorization");
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION_STORAGE.migration_invented, false);
    assert.equal(OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION_STORAGE.reused_consumed_20260925, false);
  });

  it("preserves every pin through the explicit grant", () => {
    const authorized = OPUS_V2_EVAL_V2_EXPLICIT_AUTHORIZATION;
    const prepared = OPUS_V2_EVAL_V2_PAID_AUTHORIZATION;
    assertGrantMatchesPreparedV2(OPUS_V2_EVAL_V2_EXPLICIT_GRANT, prepared);
    assert.equal(authorized.authorization_id, "reckoning-revised-13-opus-v2-full-eval-v2-20260926");
    ...
    assert.equal(
      authorized.execution_fingerprint,
      "a7417ba147afb6eb41c6baf6f7425d9c93c2b66c21652359fb8069bfc8f68321",
    );
    assert.equal(authorized.runner_id, "archivist-opus-v2-full-manuscript-eval-v2");
    assert.equal(authorized.runner_version, "archivist_opus_v2_full_manuscript_eval@v2");
    assert.equal(authorized.prompt_version, "archivist_v2_extraction_prompt@v3");
    ...
    assert.equal(OPUS_V2_EVAL_V2_COMPLETION_SUCCESS_BAR.units, "30/30");
    assert.equal(OPUS_V2_EVAL_V2_COMPLETION_SUCCESS_BAR.planned_segments_validated, "14/14");
    assert.equal(OPUS_V2_EVAL_V2_COMPLETION_SUCCESS_BAR.accepted_canon_writes, 0);
  });

  it("does not mutate or reuse the historical 20260925 authorization", () => {
    ...
    assert.equal(fenceOpusV2EvalV2PaidProvider(
      matchingOpusV2EvalV2PaidGateRequest({
  ...
  it("rejects mismatched grants and every wrong pin at $0", () => {
    const prepared = cloneOpusV2EvalV2PaidAuthorization();
    const mismatches: Array<[string, Partial<typeof OPUS_V2_EVAL_V2_EXPLICIT_GRANT>]> = [
      ["authorization_id_mismatch", { authorization_id: "reckoning-revised-13-opus-v2-full-eval-20260925" }],
      ...
      ["runner_mismatch", { runner_id: "archivist-opus-v2-full-manuscript-eval" }],
      ["workflow_kind_mismatch", { workflow_kind: "archivist-segmented-pilot" }],
      ["prompt_version_mismatch", { prompt_version: "archivist_v2_extraction_prompt@v2" }],
    ...
    for (const [reason, patch] of mismatches) {
      assert.throws(
        () =>
          authorizeOpusV2EvalV2PaidAuthorization(prepared, {
            ...OPUS_V2_EVAL_V2_EXPLICIT_GRANT,
            ...patch,
          }),
        (error: unknown) => error instanceof OpusV2EvalV2PaidAuthorizationError && error.reason === reason,
      );
    }
    assert.throws(
      () =>
  ...
  it("projects consume/resume/revoke in memory without consuming the derived grant", () => {
    ...
    const consumed = projectOpusV2EvalV2PaidAuthorization(authorized, "consume", {
    ...
    assert.throws(
      () =>
        projectOpusV2EvalV2PaidAuthorization(consumed, "consume", {
    ...
    assert.throws(
      () =>
        projectOpusV2EvalV2PaidAuthorization(authorized, "consume", {
    ...
    assert.throws(
      () =>
        projectOpusV2EvalV2PaidAuthorization(authorized, "consume", {
          workflow_id: OPUS_V2_EVAL_V2_HISTORICAL_REVISED_13_WORKFLOW_ID,
        }),
      /historical_workflow_reuse_forbidden/,
    );
    assert.doesNotThrow(() =>
    ...
    const revoked = projectOpusV2EvalV2PaidAuthorization(authorizedFromGrant(), "revoke");
  ...
  it("enforces the $5 cost gate without swallowing a later call", () => {
    assert.doesNotThrow(() =>
      assertOpusV2EvalV2PaidCostGate({ accrued_usd: 3.7, next_call_high_usd: 1.3 }),
    );
    assert.throws(
      () => assertOpusV2EvalV2PaidCostGate({ accrued_usd: 4.8, next_call_high_usd: 0.3 }),
      /cost_gate/,
    );
  });
