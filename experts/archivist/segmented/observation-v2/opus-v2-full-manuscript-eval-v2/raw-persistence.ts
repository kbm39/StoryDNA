/**
 * Experimental raw provider-output archive for a future Opus eval @v2 rerun.
...
import {
  ...
  OPUS_V2_EVAL_V2_RAW_ARCHIVE_KIND,
  OPUS_V2_EVAL_V2_RAW_ARCHIVE_RELATIVE_DIR,
  OPUS_V2_EVAL_V2_RAW_GITIGNORED,
  OPUS_V2_EVAL_V2_RAW_PRODUCTION,
  OPUS_V2_EVAL_V2_RAW_PUBLIC,
  OPUS_V2_EVAL_V2_SCHEMA_VERSION,
  OPUS_V2_EVAL_V2_SOURCE_PIN,
  OPUS_V2_EVAL_V2_VERSION,
} from "./lock.ts";

export interface OpusV2EvalV2RawCallRecord {
  workflow_id: string;
  call_id: string;
...
export function experimentalRawArchiveDir(root = process.cwd()): string {
  return join(root, OPUS_V2_EVAL_V2_RAW_ARCHIVE_RELATIVE_DIR);
}
...
export function assertRawArchivePrivacy(): void {
  if (!OPUS_V2_EVAL_V2_RAW_GITIGNORED) {
    throw new Error("opus_v2_eval_v2_raw_must_be_gitignored");
  }
  if (OPUS_V2_EVAL_V2_RAW_PUBLIC || OPUS_V2_EVAL_V2_RAW_PRODUCTION) {
    throw new Error("opus_v2_eval_v2_raw_must_stay_private");
  }
  if (OPUS_V2_EVAL_V2_RAW_ARCHIVE_KIND !== "experimental_local_calibration_artifact") {
    throw new Error("opus_v2_eval_v2_raw_archive_kind");
  }
}
...
export function persistExperimentalRawProviderOutput(
  record: OpusV2EvalV2RawCallRecord,
  root = process.cwd(),
): { path: string; raw_sha256: string } {
  assertRawArchivePrivacy();
  if (record.accepted_canon !== false) {
    throw new Error("opus_v2_eval_v2_raw_cannot_be_accepted_canon");
  }
  if (record.manuscript_id !== OPUS_V2_EVAL_V2_SOURCE_PIN.manuscript_id) {
    throw new Error("opus_v2_eval_v2_raw_source_pin_mismatch");
  }
  if (record.prompt_version !== OPUS_V2_EVAL_V2_PROMPT_VERSION) {
    throw new Error("opus_v2_eval_v2_raw_prompt_mismatch");
  }
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        archive_kind: OPUS_V2_EVAL_V2_RAW_ARCHIVE_KIND,
        public: false,
        production: false,
        raw_sha256,
        ...record,
