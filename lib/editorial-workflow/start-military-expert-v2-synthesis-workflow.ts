...
import { triggerMilitaryExpertV2SynthesisTask } from "./trigger-client.ts";
import { workflowMetadataForType } from "./workflow-definitions.ts";
import {
  MILITARY_EXPERT_V2_SYNTHESIS_DEFINITION_VERSION,
...
export interface StartMilitaryExpertV2SynthesisResult {
  ok: boolean;
  workflowId?: string;
  existing?: boolean;
  error?: string;
...
export async function startMilitaryExpertV2SynthesisWorkflow(input: {
  selectionSnapshotId: string;
  phase2aWorkflowId?: string;
}): Promise<StartMilitaryExpertV2SynthesisResult> {
  ...
  if (!handoff.ok || !handoff.inventory || !handoff.selectedSceneIds || !handoff.phase2aWorkflowId) {
  ...
  const { data: existingWorkflow } = await supabase
    ...
    .contains("input_snapshot", { phase2b: { selectionSnapshotId: input.selectionSnapshotId } })
  ...
  if (existing) {
    const snapId = existing.input_snapshot.phase2b?.selectionSnapshotId;
  ...
  const triggerResult = await triggerMilitaryExpertV2SynthesisTask(workflow.id);
  if (!triggerResult.ok) {
