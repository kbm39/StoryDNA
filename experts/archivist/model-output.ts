...
export const ARCHIVIST_MODEL_OUTPUT_CONTRACT = [
  ...
  `    "temporal_analysis": { "relation": "${ARCHIVIST_OBSERVATION_TEMPORAL_RELATIONS.join("|")}", "explanation": "string", "current_scope": { "kind": "at|from_to|as_of_book|unknown" }, "conflicting_scope": {} },`,
  '    "explanation": "string",',
  '    "suggested_resolution": "string"',
  "  }],",
  "  \"canon_delta\": [{",
  '    "id": "string",',
  `    "entity": { "alias": "string", "entity_type": "${ARCHIVIST_ENTITY_TYPES.join("|")}", "canonical_name": "string?" },`,
...
export function buildArchivistModelOutputInstructions(): string {
  return [
    ...
    `temporal_analysis.relation is observation time only: ${ARCHIVIST_OBSERVATION_TEMPORAL_RELATIONS.join("|")}. Different chapters are earlier_later, not a reason to reject a contradiction.`,
    "Persistent facts (eye color, birth date, parentage, identity, biographical history) remain contradictory across chapters unless transition evidence exists.",
    "Changeable facts (hair, rank, location, unique-object possession after transfer, alive/dead, injury condition, relationship status, age) may both be valid when chronology and transition evidence exist.",
    "Injury laterality of the same wound is an event attribute. Left vs right without a second-injury explanation is not a compatible change.",
    "Knowledge used before the established acquisition is incompatible unless another source is evidenced.",
    "classification is advisory. StoryDNA owns confirmation eligibility and the final classification.",
    `StoryDNA evaluates continuity compatibility (${ARCHIVIST_CONTINUITY_COMPATIBILITIES.join("|")}). Do not emit that field.`,
    "Do not emit entity_id or candidate_entities IDs. StoryDNA resolves names/aliases against the entity layer.",
    `Entity resolution values ${ARCHIVIST_ENTITY_RESOLUTIONS.join("|")} are StoryDNA-owned after resolution.`,
    ...
    "Leave aliases unresolved when identity is ambiguous. Do not guess. Do not fabricate candidate identities.",
