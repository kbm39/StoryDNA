...
import { buildShadowPreviewFilename } from "@/lib/studio/shadow-export.ts";
...
export async function GET(
  ...
  const body = JSON.stringify(result.shadow, null, 2);
  const filename = buildShadowPreviewFilename(result.shadow.manuscript.title, result.shadow.generatedAt, "json");
