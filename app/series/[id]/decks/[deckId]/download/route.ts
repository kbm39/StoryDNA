...
function safeName(title: string): string {
  return (title || "series-pitch-deck").replace(/[^a-zA-Z0-9._-]/g, "_").replace(/_+/g, "_").slice(0, 80);
}
...
export async function GET(
  ...
  const title = deck.title || "Series Pitch Deck";
  const buffer = await buildPitchDeckPptx(deck.content, title);

  return new Response(new Uint8Array(buffer), {
    headers: {
      "Content-Type": PPTX_MIME,
      "Content-Disposition": `attachment; filename="${safeName(title)}_Series_Pitch_Deck.pptx"`,
