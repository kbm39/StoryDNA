...
import { LITERARY_AGENT, buildSystemPrompt } from "@/lib/ai/review-engine.ts";
...
async function loadParityModuleNamespace(
  ...
  if (moduleId === "@/lib/editorial-generation/replacement-payload") {
    ...
    return {
      ...mod,
      buildReplacementPayload: (args: { issues: ParsedIssue[]; manuscriptText: string }) =>
...
export async function runLiteraryAgentDeterministicParity(
  ...
  if (!engineResult.ok) {
    const parityStatus =
      engineResult.code === "aborted" || engineResult.code === "timeout"
        ? "executor_failed"
        : "executor_failed";
    return failureResult(
      input,
