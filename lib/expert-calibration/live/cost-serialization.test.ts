...
describe("deterministic USD cost serialization", () => {
  ...
  it("2 three-call total serializes to 0.0507", () => {
    ...
    assert.equal(sumSerializedUsd(plan.calls.map((c) => c.estimatedCostUsd)), 0.0507);
  });
  ...
  it("3 JSON output contains no floating-point noise", () => {
    ...
    assert.equal(json, '{"estimated_cost_usd":0.0507}');
  });

  it("4 micro-USD total for smoke plan is 50700", () => {
    const plan = buildLiveCalibrationCallPlan({
      args: smokeArgs(),
      providerSpec: resolveProviderSpec("anthropic", ANTHROPIC_HAIKU_45_ALIAS),
      correlationPrefix: "cost-ser",
    });
    assert.equal(usdToMicroUsd(plan.totalEstimatedCostUsd), 50_700);
  });
  ...
  it("5 dry-run manifest serializes cost deterministically", async () => {
    ...
    assert.equal(result.manifest.estimated_cost_usd, 0.0507);
    const manifestJson = JSON.stringify(result.manifest);
    assert.match(manifestJson, /"estimated_cost_usd":0\.0507/);
